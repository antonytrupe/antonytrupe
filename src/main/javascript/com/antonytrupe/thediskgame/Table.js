...
function Table(description, maxPlayers, maxPoints, activations, startingDisks,
     ...
     * @memberOf Table
     */
    this.maxPoints = maxPoints || 50;
    /**
     * @memberOf Table
     */
    this.activations = activations || 3;
    /**
     * @memberOf Table
     */
    this.startingDisks = startingDisks || 6;
    /**
     * @memberOf Table
     */
    this.reinforcements = reinforcements || 3;
    /**
     * @memberOf Table
     */
    // None, Single, Neutral
    ...
    /**
     * @memberOf Table - has the data about the table
     */
    this.memento = {};
     ...
     */
    this.memento.segment = Table.SEGMENT.JOIN;
    ...
    this.activateDisk2 = function(diskNumber, playerName) {
        ...
        if (!$this.hasUnactivatedDisks(playerName)) {
            ...
            $this.getPlayerInfo(playerName).segment = Table.SEGMENT.MISSILE;
        }
        ...
        if (parseInt($this.getPlayerInfo(playerName).activations, 10) === parseInt(
                $this.activations, 10)
                || $this.getPlayerInfo($this.getCurrentPlayer()).segment !== Table.SEGMENT.ACTIVATION) {
            // clear activations
            ...
            while ($this.anyPlayersIn(Table.SEGMENT.ACTIVATION)
                    && ($this.getPlayerInfo($this.getCurrentPlayer()).segment !== Table.SEGMENT.ACTIVATION || !$this
                            .hasUnactivatedDisks($this.getCurrentPlayer()))) {

                $this.getPlayerInfo($this.getCurrentPlayer()).segment = Table.SEGMENT.MISSILE;
                $this.memento.currentPlayer = $this.getNextPlayer();
            }
        ...
        if (!$this.anyPlayersIn(Table.SEGMENT.ACTIVATION)) {
     ...
     * @returns {Array}
     */
    this.getPlayersIn = function(segment) {
        return Object.keys($this.memento.players).filter(function(pName) {
            if ($this.getPlayerInfo(pName).segment === segment) {
                return true;
            }
        });
    ...
    this.canActivate = function(playerName, diskNumber) {
        ...
        if ($this.memento.segment !== Table.SEGMENT.ACTIVATION) {
            return false;
        }
        if ($this.getPlayerInfo(playerName).segment !== Table.SEGMENT.ACTIVATION) {
            return false;
        }
    ...
    this.canMissile = function(playerName, diskNumber, projectile) {
        ...
        if ($this.memento.segment !== Table.SEGMENT.MISSILE) {
            return false;
        }

        // make sure this player is still in the missile segment
        if ($this.getPlayerInfo(playerName).segment !== Table.SEGMENT.MISSILE) {
            return false;
        }
    ...
    this.canMove = function(playerName, diskNumber) {
        ...
        if ($this.memento.segment !== Table.SEGMENT.ACTIVATION) {
            // $this.debug("Table.canMove:" + Table.SEGMENT.ACTIVATION);
            return false;
        }
    ...
    this.canReinforce = function(playerName, diskNumber) {
        ...
        if (!($this.memento.segment === Table.SEGMENT.JOIN || $this.memento.segment === Table.SEGMENT.REINFORCEMENTS)) {
            // $this.debug('6');
            return false;
        }
    ...
    this.endReinforcements = function(playerName) {
        $this.getPlayerInfo(playerName).segment = Table.SEGMENT.ACTIVATION;

        // if all the players are done with reinforcements
        var anyPlayersInReinforcement = $this
                .anyPlayersIn(Table.SEGMENT.REINFORCEMENTS);
    ...
    this.endMissiles = function(playerName) {
        $this.getPlayerInfo(playerName).segment = Table.SEGMENT.COMBAT;
    ...
    this.startActivationSegment = function() {
        ...
        this.memento.segment = Table.SEGMENT.ACTIVATION;
        ...
        Object.keys($this.memento.players).forEach(function(player) {
            ...
            $this.getPlayerInfo(player).segment = Table.SEGMENT.ACTIVATION;
        });
    ...
    this.startCombatSegment = function() {
        ...
        this.memento.segment = Table.SEGMENT.COMBAT;
        this.memento.currentPlayer = $this.memento.firstPlayer;
    ...
    this.startMissileSegment = function() {
        ...
        this.memento.segment = Table.SEGMENT.MISSILE;
        this.memento.currentPlayer = $this.memento.firstPlayer;
        ...
        if (!$this.anyPlayersIn(Table.SEGMENT.MISSILE)) {
    ...
    this.startReinforcementSegment = function() {
        ...
        Object
                ...
                .forEach(
                        function(playerName) {
                            $this.getPlayerInfo(playerName).segment = Table.SEGMENT.REINFORCEMENTS;
        ...
        if (!$this.anyPlayersIn(Table.SEGMENT.REINFORCEMENTS)
                && Object.keys($this.memento.players).length === parseInt(
                        $this.maxPlayers, 10)) {
            $this.startActivationSegment();
        }
    ...
    this.startRemoveCountersSegment = function() {
        ...
        this.memento.segment = Table.SEGMENT.REMOVE_COUNTERS;
        ...
        Object
                ...
                .forEach(
                        function(playerName) {
                            $this.getPlayerInfo(playerName).segment = Table.SEGMENT.REMOVE_COUNTERS;
                        });
        ...
        if (winners.length > 0 || $this.getDiskNumbers().length === 0) {
            this.memento.segment = Table.SEGMENT.FINISHED;
        }

        if ($this.memento.segment !== Table.SEGMENT.FINISHED) {
