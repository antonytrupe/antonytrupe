...
public class diff_match_patch {
	...
	public static class Diff {
		...
		public int hashCode() {
			final int prime = 31;
			int result = (this.operation == null) ? 0 : this.operation
					.hashCode();
	...
	public void diff_cleanupEfficiency(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				// Equality found.
				if (thisDiff.text.length() < this.Diff_EditCost
						&& (post_ins || post_del)) {
					// Candidate found.
				...
				if (lastequality != null
						&& ((pre_ins && pre_del && post_ins && post_del) || ((lastequality
								.length() < this.Diff_EditCost / 2) && ((pre_ins ? 1
								: 0)
								+ (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1
									: 0)) == 3))) {
	...
	private int diff_cleanupSemanticScore(String one, String two) {
		...
		boolean blankLine1 = lineBreak1
				&& this.BLANKLINEEND.matcher(one).find();
		boolean blankLine2 = lineBreak2
				&& this.BLANKLINESTART.matcher(two).find();
					...
					if (diff_type == Operation.INSERT) {
						...
						diff_text = diff_text.substring(
								...
								Math.min(diff_text.length(), patch_size
										- patch.length1 - this.Patch_Margin));
						patch.length1 += diff_text.length();
						start1 += diff_text.length();
